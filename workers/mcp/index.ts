...
function mcpResult(result: Record<string, unknown>) {
	if ("error" in result) {
		return { content: [{ type: "text" as const, text: JSON.stringify(result, null, 2) }], isError: true as const };
	}
	return mcpText(result);
}

/**
 * Create and register all email MCP tools on a McpServer instance.
 * Stateless — no Cloudflare Durable Object bindings required.
 */
export function createEmailMcpServer(env: Env): McpServer {
	const server = new McpServer({ name: "agentic-inbox", version: "1.0.0" });

	const verifyMailbox = (mailboxId: string) => {
		if (!fs.existsSync(path.join(process.cwd(), "data/storage", `mailboxes/${mailboxId}.json`))) {
			return mcpError(`Mailbox "${mailboxId}" not found. Use list_mailboxes to see available mailboxes.`);
		}
		return null;
	};

	server.tool("list_mailboxes", "List all available mailboxes", {}, async () => {
		return mcpText(await toolListMailboxes(env));
	});
	...
	server.tool(
		...
		async ({ mailboxId, folder, limit, page }) => {
			const denied = verifyMailbox(mailboxId); if (denied) return denied;
			return mcpText(await toolListEmails(env, mailboxId, { folder, limit, page }));
		},
	);

	server.tool(
		"get_email",
		"Get a single email with its full body content.",
		{ mailboxId: z.string().describe("The mailbox email address"), emailId: z.string().describe("The email ID to retrieve") },
		async ({ mailboxId, emailId }) => {
			const denied = verifyMailbox(mailboxId); if (denied) return denied;
			const result = await toolGetEmail(env, mailboxId, emailId);
			if ("error" in result) return { content: [{ type: "text" as const, text: "Email not found" }], isError: true };
			return mcpText(result);
		},
	);
	...
	server.tool(
		...
		{ mailboxId: z.string().describe("The mailbox email address"), threadId: z.string().describe("The thread_id to retrieve all messages for") },
		async ({ mailboxId, threadId }) => {
			const denied = verifyMailbox(mailboxId); if (denied) return denied;
			return mcpText(await toolGetThread(env, mailboxId, threadId));
		},
	);
	...
	server.tool(
		...
		async ({ mailboxId, query, folder }) => {
			const denied = verifyMailbox(mailboxId); if (denied) return denied;
			return mcpText(await toolSearchEmails(env, mailboxId, { query, folder }));
		},
	);
	...
	server.tool(
		...
		async ({ mailboxId, originalEmailId, to, subject, bodyHtml }) => {
			const denied = verifyMailbox(mailboxId); if (denied) return denied;
			return mcpResult(await toolDraftReply(env, mailboxId, { originalEmailId, to, subject, body: bodyHtml, isPlainText: false, runVerifyDraft: true }));
		},
	);
	...
	server.tool(
		...
		"Create a new draft email. Can be a new email or a reply draft.",
		{
			...
			in_reply_to: z.string().optional().describe("The ID of the email this draft is replying to (optional)"),
		...
		async ({ mailboxId, to, subject, bodyHtml, in_reply_to, thread_id }) => {
			const denied = verifyMailbox(mailboxId); if (denied) return denied;
			const result = await toolDraftEmail(env, mailboxId, { to: to || "", subject, body: bodyHtml, isPlainText: false, runVerifyDraft: true, in_reply_to, thread_id });
			if ("error" in result) return mcpResult(result);
			return mcpText({ status: "draft_created", draftId: result.draftId, threadId: result.threadId, message: "Draft created in Drafts folder." });
		},
	);
	...
	server.tool(
		...
		async ({ mailboxId, draftId, to, subject, bodyHtml }) => {
			const denied = verifyMailbox(mailboxId); if (denied) return denied;
			const result = await toolUpdateDraft(env, mailboxId, { draftId, to, subject, bodyHtml });
			if ("error" in result) {
				if (result.error === "Draft not found") return { content: [{ type: "text" as const, text: "Draft not found" }], isError: true };
	...
	server.tool(
		...
		{ mailboxId: z.string().describe("The mailbox email address"), emailId: z.string().describe("The email ID to delete") },
		async ({ mailboxId, emailId }) => {
			const denied = verifyMailbox(mailboxId); if (denied) return denied;
			return mcpResult(await toolDeleteEmail(env, mailboxId, emailId));
		},
	);
	...
	server.tool(
		...
		async ({ mailboxId, originalEmailId, to, subject, bodyHtml }) => {
			const denied = verifyMailbox(mailboxId); if (denied) return denied;
			const result = await toolSendReply(env, mailboxId, { originalEmailId, to, subject, bodyHtml });
			if ("error" in result) {
	...
	server.tool(
		...
		async ({ mailboxId, to, subject, bodyHtml }) => {
			const denied = verifyMailbox(mailboxId); if (denied) return denied;
			const result = await toolSendEmail(env, mailboxId, { to, subject, bodyHtml });
			if ("error" in result) {
				if (typeof result.error === "string" && result.error.startsWith("Failed to send"))
					return { content: [{ type: "text" as const, text: result.error }], isError: true };
				return mcpResult(result);
	...
	server.tool(
		...
		async ({ mailboxId, emailId, read }) => {
			const denied = verifyMailbox(mailboxId); if (denied) return denied;
			return mcpText(await toolMarkEmailRead(env, mailboxId, emailId, read));
		},
	);
	...
	server.tool(
		...
		async ({ mailboxId, emailId, folderId }) => {
			...
			if ("error" in result) return { content: [{ type: "text" as const, text: JSON.stringify({ error: "Failed to move email" }) }], isError: true };
			return mcpText(result);
		},
	);
...
/**
 * Handle an MCP HTTP request — stateless, no Cloudflare Durable Object required.
 * Creates a fresh server+transport per request.
 */
export async function handleMcpRequest(env: Env, request: Request): Promise<Response> {
	const transport = new WebStandardStreamableHTTPServerTransport({ sessionIdGenerator: undefined });
	const server = createEmailMcpServer(env);
